...
def run_spec_generation(project_path: Path, catalog_path: Path) -> Dict[str, Any]:
    """
    ...
    for equipment in equipment_inputs:
        ...
        required_m3_h = find_required_airflow(equipment_id, results_data, equipment)
        ...
        alternatives = selector.get_alternatives(
            eligible_models, selected_model, required_m3_h, max_count=3
        )
...
def derive_equipment_extractor_type(
    equipment: Dict[str, Any], input_data: Dict[str, Any]
) -> str:
    """Derive equipment extractor type from served areas."""
    area_types = {
        area["id"]: area.get("extractor_type") for area in input_data.get("areas", [])
    }
    served_types = {
        area_types[area_id]
...
def find_required_airflow(
    ...
    area_results = {
        area_result["area_id"]: area_result
        for area_result in results_data.get("area_results", [])
    }
    served_area_ids = equipment.get("serves_area_ids", [])
    assigned_airflows = [
        area_results[area_id].get("required_m3_h_final")
        for area_id in served_area_ids
        if area_id in area_results
    ]
    if not assigned_airflows:
        return None
    return round(sum(assigned_airflows), 2)
