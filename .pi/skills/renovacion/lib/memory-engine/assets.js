...
class AssetManager {
  ...
  <text x="200" y="160" font-family="Arial" font-size="12" fill="#aaa" text-anchor="middle">Imagen no disponible</text>
		...
		await fs.writeFile(
			path.join(this.placeholdersDir, "placeholder-logo.svg"),
			logoPlaceholder,
		);
		await fs.writeFile(
			path.join(this.placeholdersDir, "placeholder-equipment.svg"),
	...
	async resolveLogo(logoRef, type = "empresa") {
		...
		if (logoRef.startsWith("http://") || logoRef.startsWith("https://")) {
			try {
				...
				this.warnings.push(
					`Failed to download ${type} logo from ${logoRef}: ${err.message}`,
				);
				return null;
			}
		}
		...
		try {
			...
			this.warnings.push(
				`Failed to copy ${type} logo from ${logoRef}: ${err.message}`,
			);
			return null;
		}
	}
	...
	async resolveEquipmentImage(imageRef) {
		...
		try {
			...
			try {
				...
				this.warnings.push(
					`Failed to resolve equipment image ${imageRef}: ${err.message}`,
				);
				return "assets/placeholders/placeholder-equipment.svg";
			}
		}
