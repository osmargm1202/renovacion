    ...
    <div class="equipment-body">
      ...
      <div class="equipment-specs">
        ...
        <div class="equipment-spec-row">
          ...
          <div class="equipment-spec-value"><strong>${formatAirflow(selected_model.airflow_m3_h)}</strong></div>
        </div>
...
function renderAlternatives(alternatives, equipAssets) {
  ...
  const rows = alternatives.map(alt => `
    <tr>
      ...
      <td class="text-right">${formatAirflow(alt.airflow_m3_h)}</td>
...
function formatAirflow(m3h) {
  if (m3h === null || m3h === undefined || Number.isNaN(Number(m3h))) return 'N/A';
  return `${Number(m3h).toFixed(2)} m³/h (${m3hToCfm(m3h).toFixed(2)} CFM)`;
}
